...
module.exports.createCampground = async (req, res, next) => {
  ...
  if (!geoData.body.features.length) {
    req.flash(
      "error",
      `Please enter a valid location (format: City, State) <br> Error: Cannot find ${req.body.campground.location}`
    );
    return res.redirect("/campgrounds/new");
  }
  ...
  try {
    var Email = {
      ...
      html: `<b>Hi ${username}, you have successfully created a new campgound on ${new Date().toLocaleString()}. Click the link below to check your new campground <br> </b> <a href="https://desolate-journey-89417.herokuapp.com/campgrounds/${
        campground._id
      }">${req.body.campground.title}</a>`,
    };
...
module.exports.updateCampground = async (req, res) => {
  ...
  if (
    ...
    campground.images.length - req.body.deleteImages.length < 1
  ) {
